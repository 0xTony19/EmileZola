/**
 * Standard QRCode.js (Davidshimjs) - 100% Offline Standard ISO/IEC 18004 QR Generator
 * Garantisce la scansione immediata da QUALSIASI fotocamera di smartphone (iOS, Android, Google Lens).
 */
var QRCode;
(function () {
	...
	QRCode.prototype.draw = function () {
		...
		var nWidth = Math.floor(size / count);
		var nHeight = Math.floor(size / count);
		var nAdjust = Math.floor((size - (nWidth * count)) / 2);
		...
		for (var row = 0; row < count; row++) {
			for (var col = 0; col < count; col++) {
				if (this._oQRCode.isDark(row, col)) {
					ctx.fillRect(nAdjust + col * nWidth, nAdjust + row * nHeight, nWidth, nHeight);
				}
			}
		}
	};
